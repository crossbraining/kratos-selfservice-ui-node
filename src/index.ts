...
if (Boolean(config.hydra.admin)) {
  ...
  app.get('/auth/hydra/consent', protect, csrfProtection, hydraGetConsent, errorHandler);
  app.post('/auth/hydra/consent', protect, bodyParser.urlencoded({ extended: true }), csrfProtection, hydraPostConsent);
}
//// Hydra End ////
...
let listener = () => {
  ...
  console.info(`Security mode: ${config.securityMode}`);
};
